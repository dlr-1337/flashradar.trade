...
$payload = pj_history_sync_step(true);
...
assert_same(1, (int) ($payload['counts']['preserved'] ?? -1), 'Should preserve the legacy row untouched.');
assert_same(3, (int) ($payload['counts']['total_json_rows'] ?? -1), 'Merged JSON row count should include preserved + managed rows.');

$savedRows = json_decode((string) file_get_contents($jsonFile), true, 512, JSON_THROW_ON_ERROR);
assert_same(3, count($savedRows), 'dados.json should be rewritten with the merged rows.');
...
$completedPayload = pj_history_sync_step(false);
...
assert_contains('2025-12-01', (string) ($historicalEventCalls[0]['query']['from'] ?? ''), 'Historical windows should start at December 2025.');
